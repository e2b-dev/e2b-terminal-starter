...
function sessionSecret() {
  ...
  if (!secret) {
    throw new Error(
      "Missing APP_SESSION_SECRET or E2B_API_KEY for session signing.",
    );
  }
  return secret;
}
...
function openUserId(value: string | undefined) {
  ...
  const signatureBuffer = Buffer.from(value.slice(separator + 1));
  const expectedBuffer = Buffer.from(sign(userId));
  if (
...
export function setCurrentUser(response: NextResponse, userId: string) {
